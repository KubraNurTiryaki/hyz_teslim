...
bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings)
{
    ...
    if(!node.empty())
    {
        fIniThFAST = node.operator float();
    }
    else
    {
    ...
    if(!node.empty())
    {
        fMinThFAST = node.operator float();
    }
    else
    {
    ...
    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR) {
        // Init çıkarıcısı 5*nFeatures üretiyordu (hedef 4000; logda 2600-3600
        // gözlendi); trace'li LightGlue ~1600 nokta üstünde bozuluyor -> init
        // hiç tutmuyordu (815 karelik ölü bölge). LightGlue güvenli bölgesi: <=1200.
        const int nIniFeat = (5*nFeatures < 1200) ? 5*nFeatures : 1200;
        mpIniORBextractor = new SPextractor(nIniFeat,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
