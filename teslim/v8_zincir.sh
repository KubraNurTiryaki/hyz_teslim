#!/bin/bash
# v8 doğrulama zinciri: O2, O3, O4 sıralı (saf SLAM — bridge.py --blend-tau
...
kos() {
    ...
    echo "=== Oturum $OT (v8) basliyor: $(date +%H:%M:%S) ==="
    rm -rf prova2025/run_O${OT}v8
    rm -f prova2025/pred_O${OT}v8.csv prova2025/mock_O${OT}v8.csv
    $PY mock_server.py --frames-dir $FR --gt prova2025/oturum${OT}_gt.csv \
        --start 0 --limit 2250 --health-drop "$DROP" --paced --fps 4 \
        --port $PORT --out prova2025/mock_O${OT}v8.csv &
    local MOCK=$!
    sleep 2
    $PY bridge.py --server 127.0.0.1:$PORT \
        --settings $HOME/SP_SLAM3/Examples/Monocular/$YML \
        --run-dir prova2025/run_O${OT}v8 --out prova2025/pred_O${OT}v8.csv
    wait $MOCK 2>/dev/null
    echo "=== Oturum $OT (v8) bitti: $(date +%H:%M:%S) ==="
}
